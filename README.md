...
[![Build Status](https://travis-ci.org/mozilla-appmaker/component-header.png)](https://travis-ci.org/mozilla-appmaker/component-header)

Basic button component for [Appmaker](https://github.com/mozilla-appmaker/appmaker).

Appmaker import:
```
<link rel="import" href="/component-header">
```

# Test Status

See the github page http://mozilla-appmaker.github.io/component-header/ for test status.
...
Clone this repo:
```
git clone git@github.com:mozilla-appmaker/component-header.git
```
...
Your directory structure should look like this (more component repos can be cloned as siblings of `component-header`):
```
mozilla-appmaker/
  ├── appmaker/
  ├── component-header/
...
Enter the `component-header` directory, and install npm dependencies
```
cd component-header
npm install
```
...
```
  ...
  beforeEach(function (done) {
    iframeHandler = harnessUtils.createIframe('test/html/test.html', function (win, doc) {
      buttonElement = iframeHandler.document.querySelector('ceci-header');
      done();
    });
  });
...
<body>
  ...
  <ceci-header id="ceci-header" value="you must construct additional pylons"></ceci-header>
</body>
</html>
```
...
<script>
  iframeTestUtils(function () {
    test('example', function (done) {
      var buttonElement = document.querySelector('ceci-header');
